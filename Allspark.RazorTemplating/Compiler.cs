...
namespace Allspark.RazorTemplating
{
    public class Compiler
    {
        private static GeneratorResults GenerateCode(RazorTemplateEntry entry)
        {
            ...
            {
                DefaultBaseClass =
                    string.Format("RazorTemplateingSample.RazorTemplating.RazorTemplateBase<{0}>",
                        entry.ModelType.FullName),
                DefaultNamespace = "RazorTemplateingSample",
                DefaultClassName = entry.TemplateName + "Template"
            };
